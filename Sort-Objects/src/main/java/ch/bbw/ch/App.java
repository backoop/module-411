...
public class App 
{
    public static void main( String[] args )
    {
        ...
        comparator.compare(persons[1], persons[2]);
        //injection sort
        for (int i = 0; i < persons.length; i++) {
            sortedPersons[i] = persons[i];
            for (int j = i; j > 0; j--) {
                if (comparator.compare(sortedPersons[j - 1], sortedPersons[j]) > 0) {
                    Person temp = sortedPersons[j - 1];
                    sortedPersons[j - 1] = sortedPersons[j];
                    sortedPersons[j] = temp;
                }
            }
        }

        for (int i = 0; i < sortedPersons.length; i++) {
            System.out.println(sortedPersons[i]);
        }
